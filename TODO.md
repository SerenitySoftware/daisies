...
- [ ] **Delete the stale local `release/0.3.0` branch.** Its PR ([#15](https://github.com/SerenitySoftware/daisies/pull/15)) was squash-merged as `b37f74a`, so its content is fully in `master` — `git diff master release/0.3.0` is deletions only — but its own commits never became ancestors, which is why `git branch -d` refuses it. It exists only locally; origin deleted it at merge time. Needs `git branch -D release/0.3.0`, which the unattended TODO routine won't run on its own.
